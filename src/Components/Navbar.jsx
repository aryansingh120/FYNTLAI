...
const Navbar = ({navItems}) => {
  ...
  return (
   <div className='sm:px-[7vw] px-[1vw] pt-[1rem]'>
    <nav className=' flex border justify-between rounded-2xl py-[.8rem] px-[1vw] shadow-lg'>
        ...
        <ul className='flex flex-col sm:flex-row items-center gap-[4vw] mt-[3rem] sm:mt-0 pr-[6rem] sm:pr-0'>
            {
                navItems.map((items,index)=>{
                    return (
                        <Link to={`/${items}`} >
                            <li className='text-[#525151] font-medium capitalize' key={index}>{items}</li>
                        </Link>
                    )
                })
            }
